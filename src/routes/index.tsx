...
import { getTopFavoritedBooks } from "@/lib/api/books.functions";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "γραπτός — A cinematic digital library" },
      { name: "description", content: "Read timeless public-domain classics in a beautifully crafted reading room." },
      { property: "og:title", content: "γραπτός — A cinematic digital library" },
...
function Index() {
  ...
  useEffect(() => {
    if (featuredIndex >= books.length && books.length > 0) {
      setFeaturedIndex(0);
    }
    ...
    <div>
      {featured && !hasFilters && (
        <section className="relative overflow-hidden">
          ...
          <div className="relative max-w-7xl mx-auto px-6 py-6 md:py-32 grid md:grid-cols-[1fr_260px] gap-10 items-center">
            <div className="animate-fade-in text-center md:text-left" key={featured.id + "-text"}>
              ...
              <div className="flex items-center justify-center md:justify-start gap-2 mb-6">
                {books.length > 1 && books.slice(0, Math.min(books.length, 10)).map((_, idx) => (
                  <button
                    key={idx}
            ...
            {topFavorited.length > 0 && (
              <div className="animate-fade-in-up">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <span className="text-xs text-muted-foreground ml-auto">Popular among readers</span>
                </div>
