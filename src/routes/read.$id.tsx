...
import { ArrowLeft, Bookmark as BookmarkIcon, Plus, X, BookOpen, Settings, Type, ExternalLink } from "lucide-react";
...
function Reader() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ error }: { error: any }) => {
        if (!error) qc.invalidateQueries({ queryKey: ["reading-progress", book.id, user.id] });
      });
  ...
  const updateProgress = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(`Progress saved at page ${page}`);
    },
    onError: (e: Error) => toast.error(e.message),
  });

  const isPdf = book.source === "pdf";

  // Redirect external books to their URL
  useEffect(() => {
    if (!isPdf && book.content_url) {
      window.location.href = book.content_url;
    }
  }, [book.content_url, isPdf]);
    ...
    <div className={`fixed inset-0 flex flex-col ${themeStyles} transition-colors duration-300`}>
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex-1 min-w-0 relative">
          {isPdf ? (
            <embed
              src={`${book.content_url}#toolbar=1&navpanes=0`}
            ...
            <div className="w-full h-full flex items-center justify-center p-8">
              <div className="max-w-md text-center space-y-4">
                ...
                <p className="text-sm text-muted-foreground">Redirecting to book...</p>
              </div>
            </div>
          )}
        </div>
