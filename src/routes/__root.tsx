...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { title: "Lumen — Read the classics, beautifully" },
      { name: "description", content: "A cinematic digital library of public-domain classics. Browse, read, and bookmark timeless books." },
      { name: "keywords", content: "digital library, public domain books, classic literature, free ebooks, reading app, online library" },
      ...
      { name: "twitter:creator", content: "@lumenapp" },
...
function RootComponent() {
  ...
  useEffect(() => {
    const { data: sub } = supabase.auth.onAuthStateChange((_event: string) => {
      if (_event !== "SIGNED_IN" && _event !== "SIGNED_OUT" && _event !== "USER_UPDATED") return;
      router.invalidate();
      if (_event !== "SIGNED_OUT") queryClient.invalidateQueries();
    });
